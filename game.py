...
def normalise_input(user_input):
    ...
    normalise = ""
    normalise = remove_punct(user_input)
    normalise = remove_spaces(normalise)
    normalise = normalise.lower()
    return normalise
...
def menu(exits):
    ...
    while True:
        print_menu(exits)
        user = normalise_input(input())
        valid = is_valid_exit(exits,user)
        if valid == True:
            return user
        else:
